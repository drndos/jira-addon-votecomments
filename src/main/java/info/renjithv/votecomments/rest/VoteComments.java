...
public class VoteComments {
  ...
  public VoteComments(ActiveObjects ao, IssueManager issueManager, PermissionManager permissionManager,
  ...
  public Response getIssueCommentsVotes(@QueryParam("issueid") final Long issueid) {
    ...
    if (null != issueObject && permissionManager.hasPermission(Permissions.VIEW_VOTERS_AND_WATCHERS, issueObject,
        loggedInUser)) {
      ao.executeInTransaction(new TransactionCallback<Void>() {
        ...
        public Void doInTransaction() {
          ...
          for (VoteInfo voteInfo : votes) {
            ...
            VoteCommentsModel inData = new VoteCommentsModel(voteInfo.getCommentId(), 0, 0, new ArrayList<String>(),
  ...
  @Produces({MediaType.APPLICATION_JSON})
  @Path("upvote")
  public Response upvoteComment(@QueryParam("commentid") final Long commentid,
      @QueryParam("issueid") final Long issueid) {
    ...
    return Response.ok(new VoteCommentsModel(commentid, 0, 0, new ArrayList<String>(), new ArrayList<String>()))
        .build();
  }
