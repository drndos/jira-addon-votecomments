function AddVoteButtons() {
    ...
    AJS.$(this).find('.action-links').each(function () {
      if (AJS.$(this).find('.upvote').length === 0) {
        ...
        if (loggedInUser !== commentUser) {
          ...
          buttonsAdded = true;
          AJS.$(this).append(AJS.$(
              '<a href="#" id="upvote" class="upvote" commentid=' + commentId
              + ' title="Up votes this comment">' +
              '<img class="emoticon" src="' + AJS.contextPath()
              + '/images/icons/emoticons/thumbs_up.gif" height="16" width="16" align="absmiddle" alt=""></a>'
              +
              '<a href="#" id="downvote" class="downvote" commentid=' + commentId
              + ' title="Down votes this comment">' +
              '<img class="emoticon" src="' + AJS.contextPath()
              + '/images/icons/emoticons/thumbs_down.gif" height="16" width="16" align="absmiddle" alt=""></a>'));
        }
      } else {
  ...
  if (buttonsAdded) {
    AJS.$('.upvote').click(function (event) {
      ...
      AJS.$.ajax({
        url: AJS.contextPath() + "/rest/votecomments/latest/upvote?commentid=" + AJS.$(this).attr(
            'commentid') + '&issueid=' + issueID,
        success: function () {
    ...
    AJS.$('.downvote').click(function (event) {
      event.preventDefault();
      AJS.$.ajax({
        url: AJS.contextPath() + "/rest/votecomments/latest/downvote?commentid=" + AJS.$(this).attr(
            'commentid') + '&issueid=' + issueID,
        success: function () {
...
function ShowCurrentVotes() {
  ...
  if (typeof issueID === "undefined") {
    ...
    AJS.$.getJSON(AJS.contextPath() + "/rest/votecomments/latest/commentsvotes?issueid=" + issueID,
        function (data) {
          ...
          AJS.$('div[id|=comment][id!=comment-wiki-edit]').each(function () {
            ...
            AJS.$(this).find('.action-links').each(function () {
              ...
              if (cmData && cmData.downvotes) {
                AJS.$(this).before(
                    AJS.$('<div class="currentvotes dislikes" title="' + cmData.dislikers.join(",")
                        + '">' + cmData.downvotes + ' dislike(s)</div>')
                );
              }
              if (cmData && cmData.upvotes) {
                AJS.$(this).before(
                    AJS.$('<div class="currentvotes likes" title="' + cmData.likers.join(",") + '">'
                        + cmData.upvotes + ' like(s)</div>')
                );
              }
            });
          });
...
function CheckPermissionsAndAdd() {
  AJS.$.getJSON(
      AJS.contextPath() + "/rest/api/latest/mypermissions?issueKey=" + AJS.Meta.get("issue-key"),
      function (data) {
